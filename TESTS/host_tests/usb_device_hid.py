...
def get_descriptor_types(desc):
    ...
    desc is expected to be a sequence of bytes, most likely array.array('B')
...
def get_usbhid_dev_type(intf):
    ...
    #   the class is specified in the Interface descriptor and not the Device descriptor.
    if not isinstance(intf, usb.core.Interface):
        return None
    if intf.bInterfaceClass != USB_CLASS_HID:
        return None
    if intf.bInterfaceSubClass == HID_SUBCLASS_BOOT and intf.bInterfaceProtocol == HID_PROTOCOL_KEYBOARD:
        return 'boot_keyboard'
    if intf.bInterfaceSubClass == HID_SUBCLASS_BOOT and intf.bInterfaceProtocol == HID_PROTOCOL_MOUSE:
        return 'boot_mouse'
...
class USBHIDTest(mbed_host_tests.BaseHostTest):
    ...
    def get_usb_hid_path(usb_id_str):
        """Get USB HID device path as registered in the system.
    ...
    def notify_error(self, msg):
        ...
        # TODO decide if False or None should be passed to notify_complete()
    ...
    def foo(self):
        ...
        hids = hid.enumerate()
        for hdev in hids:
            print('VID = {vendor_id:#06x}, PID = {product_id:#06x}, SN = {serial_number}'
                    .format(**hdev))
        inputs.devices._post_init() # update the list
        idevs = inputs.devices
        # for dev in inputs.DeviceManager():  # raises on my linux machine
        for dev in idevs:
            print(dev)
        print('DBG HOST DONE')
        return
    ...
    def cb_test_get_hid_desc(self, key, value, timestamp):
        ctrl_kwargs_for_hid_desc_req = {
            'bmRequestType': build_request_type(CTRL_IN, CTRL_TYPE_STANDARD, CTRL_RECIPIENT_INTERFACE),
            'bRequest': USB_REQUEST_GET_DESCRIPTOR,
            # Descriptor Index (part of wValue) is reset to zero for HID class descriptors other than Physical ones.
        ...
        ctrl_kwargs_for_report_desc_req = {
            'bmRequestType': build_request_type(CTRL_IN, CTRL_TYPE_STANDARD, CTRL_RECIPIENT_INTERFACE),
            'bRequest': USB_REQUEST_GET_DESCRIPTOR,
            # Descriptor Index (part of wValue) is reset to zero for HID class descriptors other than Physical ones.
        ...
        try:
            ...
            return
        cfg = mbed_hid_dev.get_active_configuration()
        try:
            for intf in cfg:
                if intf.bInterfaceClass != USB_CLASS_HID:
                    continue
                mbed_hid_dev.detach_kernel_driver(intf.bInterfaceNumber)

                # Request the HID descriptor.
                ctrl_kwargs_for_hid_desc_req['wIndex'] = intf.bInterfaceNumber
                hid_desc = mbed_hid_dev.ctrl_transfer(**ctrl_kwargs_for_hid_desc_req)
                try:
                    bNumDescriptors, bDescriptorType, wDescriptorLength = self.get_hid_descriptor_parts(hid_desc)  # pylint: disable=invalid-name
                ...
                ctrl_kwargs_for_report_desc_req['wIndex'] = intf.bInterfaceNumber
                ctrl_kwargs_for_report_desc_req['data_or_wLength'] = wDescriptorLength
                report_desc = mbed_hid_dev.ctrl_transfer(**ctrl_kwargs_for_report_desc_req)
    ...
    def cb_test_get_cfg_desc(self, key, value, timestamp):
        ctrl_kwargs_for_cfg_desc_req = {
            'bmRequestType': build_request_type(CTRL_IN, CTRL_TYPE_STANDARD, CTRL_RECIPIENT_DEVICE),
            'bRequest': USB_REQUEST_GET_DESCRIPTOR,
        ...
        try:
            ...
            cfg_desc = mbed_hid_dev.ctrl_transfer(**ctrl_kwargs_for_cfg_desc_req)
    ...
    def cb_test_class_requests(self, key, value, timestamp):
        """Verify all required requests are supported.
        ...
        """
        HID_IDLE_RATE = 0x00  # Duration = 0 (indefinite)
        HID_PROTOCOL = 0x01  # Protocol = 1 (Report Protocol)
        ctrl_kwargs_for_get_report_request = {
            'bmRequestType': build_request_type(CTRL_IN, CTRL_TYPE_CLASS, CTRL_RECIPIENT_INTERFACE),
            ...
            # wLength: unknown, set to 1024
            'data_or_wLength': 1024}
        ctrl_kwargs_for_get_idle_request = {
            'bmRequestType': build_request_type(CTRL_IN, CTRL_TYPE_CLASS, CTRL_RECIPIENT_INTERFACE),
            ...
            'wIndex': None,
            'data_or_wLength': 1}
        ctrl_kwargs_for_set_idle_request = {
            'bmRequestType': build_request_type(CTRL_OUT, CTRL_TYPE_CLASS, CTRL_RECIPIENT_INTERFACE),
            'bRequest': HID_REQUEST_SET_IDLE,
            # wValue: Duration, ReportID = 0 (all input reports)
            'wValue': (HID_IDLE_RATE << 8) | 0x00,
            # wIndex: InterfaceNumber (defined later)
            'wIndex': None,
            'data_or_wLength': 0}
        ctrl_kwargs_for_get_protocol_request = {
            'bmRequestType': build_request_type(CTRL_IN, CTRL_TYPE_CLASS, CTRL_RECIPIENT_INTERFACE),
            ...
            'wIndex': None,
            'data_or_wLength': 1}
        ctrl_kwargs_for_set_protocol_request = {
            'bmRequestType': build_request_type(CTRL_OUT, CTRL_TYPE_CLASS, CTRL_RECIPIENT_INTERFACE),
        ...
        try:
            for intf in cfg:
                ...
                if hid_dev_type == 'boot_keyboard':
                    ...
                    ctrl_kwargs_for_set_idle_request['wIndex'] = intf.bInterfaceNumber
                    mbed_hid_dev.ctrl_transfer(**ctrl_kwargs_for_set_idle_request)
                    # 3. Get_Idle
                    tested_request_name = 'Get_Idle'
                    ctrl_kwargs_for_get_idle_request['wIndex'] = intf.bInterfaceNumber
                    idle_rate = mbed_hid_dev.ctrl_transfer(**ctrl_kwargs_for_get_idle_request)
                    raise_if_different(HID_IDLE_RATE, idle_rate, 'Invalid idle rate received. ')
                if hid_dev_type in ('boot_keyboard', 'boot_mouse'):
                    # 6. Set_Protocol
                    tested_request_name = 'Set_Protocol'
                    ctrl_kwargs_for_set_protocol_request['wIndex'] = intf.bInterfaceNumber
                    mbed_hid_dev.ctrl_transfer(**ctrl_kwargs_for_set_protocol_request)
                    # 5. Get_Protocol
                    tested_request_name = 'Get_Protocol'
                    ctrl_kwargs_for_get_protocol_request['wIndex'] = intf.bInterfaceNumber
                    protocol = mbed_hid_dev.ctrl_transfer(**ctrl_kwargs_for_get_protocol_request)
                    raise_if_different(HID_PROTOCOL, protocol, 'Invalid protocol received. ')
                # 1. Get_Report
                tested_request_name = 'Get_Report'
                ctrl_kwargs_for_get_report_request['wIndex'] = intf.bInterfaceNumber
                report = mbed_hid_dev.ctrl_transfer(**ctrl_kwargs_for_get_report_request)
                print('report', repr(report))
                # TODO then make it pretty (use a dict?) OR use dedicated test functions(intf)
        except usb.core.USBError as exc:
